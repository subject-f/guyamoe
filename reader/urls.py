...
urlpatterns = [
    ...
    re_path(r'^manga/(?P<series_slug>[\w-]+)/(?P<chapter>[\d-]{1,9})/$', views.reader, name='reader-manga-chapter-shortcut'),
    re_path(r'^series/(?P<series_slug>[\w-]+)/(?P<chapter>[\d-]{1,9})/$', views.reader, name='reader-series-chapter-shortcut'),
    ...
    re_path(r'^md_proxy/(?P<md_series_id>[\d]{1,9})/(?P<chapter>[\d-]{1,12})/(?P<page>[\d]{1,9})/$', views.md_chapter, name='reader-md-chapter'),
    re_path(r'^nh_proxy/(?P<nh_series_id>[\d]{1,9})/$', views.nh_proxy, name='reader-nh-proxy'),
    re_path(r'^nh_proxy/(?P<nh_series_id>[\d]{1,9})/(?P<chapter>[\d-]{1,9})/(?P<page>[\d]{1,9})/$', views.nh_chapter, name='reader-nh-chapter'),
    re_path(r'^fs_proxy/(?P<encoded_url>[\w\d.%-]+)/$', views.fs_proxy, name='reader-fs-proxy'),
    re_path(r'^fs_proxy/(?P<encoded_url>[\w\d.%-]+)/(?P<chapter>[\d-]{1,9})/(?P<page>[\d]{1,9})/$', views.fs_chapter, name='reader-fs-chapter'),
