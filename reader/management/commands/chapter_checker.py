...
class Command(BaseCommand):
    ...
    async def jaiminis_box_checker(self, downloaded_chapters, series, latest_volume, url, latest_chap=None):
        ...
        for chapter in chapters:
            ...
            if "(Digital)" in chapters[chapter]["title"] and str(float(int(float(chapter)))) in downloaded_chapters:
                if hasattr(settings, SCRAPER_BLACKLIST_FILE) and os.path.exists(settings.SCRAPER_BLACKLIST_FILE):
