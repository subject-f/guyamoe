...
def generate_image(path):
    if not os.path.isfile(real_path(path)):
        ...
        ImageDraw.Draw(img).text((width / 2, height / 2), path.split("/")[-1], fill=(0, 0, 0))
        if "blur" in path:
    ...
    with open(path, "r") as fixtures:
        ...
        for d in data:
            ...
            if d["model"] == "reader.chapter":
                directories.append(f"manga/{series[f['series']]}/chapters/{f['folder']}/{f['group']}")
                directories.append(f"manga/{series[f['series']]}/chapters/{f['folder']}/{f['group']}_shrunk")
                directories.append(f"manga/{series[f['series']]}/chapters/{f['folder']}/{f['group']}_shrunk_blur")
                for page_num in range(1, PAGES + 1):
                    chapters.append(f"manga/{series[f['series']]}/chapters/{f['folder']}/{f['group']}/{page_num:03}.jpg")
                    chapters.append(f"manga/{series[f['series']]}/chapters/{f['folder']}/{f['group']}_shrunk/{page_num:03}.jpg")
                    chapters.append(f"manga/{series[f['series']]}/chapters/{f['folder']}/{f['group']}_shrunk_blur/{page_num:03}.jpg")
...
if __name__ == "__main__":
    ...
    with Pool(PROCESSES) as p:
        changed = [
            status
            for status in p.map(generate_image, fixtures["chapters"] + fixtures["volume_covers"])
            if status
        ]
